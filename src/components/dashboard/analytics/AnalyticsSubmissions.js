...
const AnalyticsSubmissions = () => {
  ...
  const getSubmissionInfo = async () => {
    try {
      const getSubmissionData = await API.graphql(
        graphqlOperation(queries.listFormSubmissions, {
          filter: { formID: { eq: submissionId } },
        })
      );
      const listSubmissionData =
        getSubmissionData.data.listFormSubmissions.items;
      ...
      <>
        {showResponses ? (
          ...
          <Box>
            ...
            {/* Pass each question's properties to AnalyticsGraphBar  */}
            {Object.values(JSON.parse(originalFormQuestions)).map(
              (question, idx) => (
